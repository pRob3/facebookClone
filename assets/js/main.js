$(function() {
   ...
   function messageNotificationUpdate(userid){
      $.post(BASE_URL +'core/ajax/notify.php', {
         ...
         if(data.trim() === '0'){
            $('.message-count').empty();
            $('.message-count').css({"background-color":"transparent"});
         }else{
            $('.message-count').html(data);
            $('.message-count').css({"background-color":"red"});
         }
      })
   }
   ...
   messageNotificationInterval = setInterval(function(){
      messageNotificationUpdate(u_id);
   }, 1000);
